...
| 引用图 | 支持 `xray outbound -> clash socks listener`，以及 `clash xray-socks5 upstream -> xray inbound`，并生成服务依赖顺序 |
| Xray 生成器 | 稳定输出 JSON，支持 vmess、shadowsocks、socks5、http inbound；支持 clash/socks5/http/direct outbound；支持 api/stats/policy |
| mihomo 生成器 | 稳定输出 YAML，支持 socks/http listener、raw upstream、xray-socks5 upstream、select/url-test/load-balance/fallback、默认 rules profile |
...
- 保持订阅边界：订阅只来自 `xray.inbounds[]` 中 `sub: true` 的节点，不把 clash upstream、groups、rules、controller 写入订阅。
...
- 不自动导入旧 `clash`、`xray`、`clashsub` 或旧 `proxy-stack` 目录。
