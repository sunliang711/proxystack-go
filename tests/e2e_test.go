...
func TestMigrationMainFlowE2E(t *testing.T) {
	...
	runSubCommand(t, "--base-dir", baseDir, "import", bundlePath)
	require.FileExists(t, filepath.Join(baseDir, "sub", "inputs", "usa1.yaml"))

	port := reserveTCPPort(t)
	writeSubConfig(t, filepath.Join(baseDir, "sub", "config.yaml"), port)
	...
	cancel, done := startSubServer(t, baseDir)
