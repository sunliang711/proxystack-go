...
func TestInitProjectWritesCommentedAgentConfig(t *testing.T) {
	...
	require.Contains(t, content, "# 全局订阅用户档案。stack 的 xray.inbounds[].user_refs 会引用这里的 user/profile。")
...
func TestAddCloneAndMemberCommands(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "user1", usa1.Xray.Inbounds[1].UserRefs[0].User)
	require.Equal(t, 4300, usa1.Xray.Inbounds[0].Port)
	...
	require.NoError(t, err)
	require.Equal(t, "usa2.clash.socks", usa2.Xray.Outbound.Ref)
...
func TestAddStackUsesReferenceTemplateFormat(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, 24000, stack.Xray.Inbounds[0].Port)
	require.Equal(t, "user1", stack.Xray.Inbounds[1].UserRefs[0].User)
