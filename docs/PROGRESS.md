...
- 尚未执行：真实 systemd/launchd、下载源、root owner 修复、真实 Docker build/run、真实网络 ipinfo 的部署环境手工验收。
...
- 已完成 T11-T15：agent 配置命令、runtime plan/manifest、服务生命周期与 unit/plist/metadata、install/update/self update。
...
建议下一步在目标部署环境执行真实 systemd/launchd、下载源、Docker build/run 和真实网络 ipinfo 验收；如发现行为差异，再补充对应测试和交付说明。
